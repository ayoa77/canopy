...
class PaymentVerifyBase
  ...
  @@param_xml = Nokogiri::XML(File.open(File.join(File.dirname(__FILE__), 'AllpayPayment.xml')))

  def get_svc_url(apiname, mode)
    url = @@param_xml.xpath("/allPayPayment/#{apiname}/ServiceAddress/url[@type=\"#{mode}\"]").text
    return url
  end

  def get_special_encode_param(apiname)
    ret = []
    node = @@param_xml.xpath("/allPayPayment/#{apiname}/Parameters//param[@urlencode=\"1\"]")
  ...
  def get_basic_params(apiname)
    basic_param = []
    @@param_xml.xpath("/allPayPayment/#{apiname}/Parameters/param[@require=\"1\"]").each do |elem|
  ...
  def get_cond_param(apiname)
    aio_sw_param = []
    conditional_param = {}
    @@param_xml.xpath("/allPayPayment/#{apiname}/Config/switchparam/n").each do |elem|
      aio_sw_param.push(elem.text)
    end
    aio_sw_param.each do |pname|
      opt_param = {}
      node = @@param_xml.xpath("/allPayPayment/#{apiname}/Parameters//param[@name=\"#{pname}\"]")
  ...
  def get_param_type(apiname)
    param_type = {}
    @@param_xml.xpath("/allPayPayment/#{apiname}/Parameters//param").each do |elem|
  ...
  def get_opt_param_pattern(apiname)
    pattern = {}
    node = @@param_xml.xpath("/allPayPayment/#{apiname}/Parameters//param[@type=\"Opt\"]")
  ...
  def get_int_param_pattern(apiname)
    pattern = {}
    node = @@param_xml.xpath("/allPayPayment/#{apiname}/Parameters//param[@type=\"Int\"]")
    node.each do |param_elem|
  ...
  def get_str_param_pattern(apiname)
    pattern = {}
    node = @@param_xml.xpath("/allPayPayment/#{apiname}/Parameters//param[@type=\"String\"]")
    node.each do |param_elem|
  ...
  def get_depopt_param_pattern(apiname)
    ...
    node = @@param_xml.xpath("/allPayPayment/#{apiname}/Parameters//param[@type=\"DepOpt\"]")
    node.each do |param_elem|
      parent_n_opts = {}
      sub_opts = {}
      p_name = param_elem.attributes['name'].value
  ...
  def verify_param_by_pattern(params, pattern)
    ...
    params.keys.each do |p_name|
      ...
      when p_type == 'String'
        ...
        if mat.nil?
          raise InvalidParam, "Wrong format of param #{p_name} or length exceeded."
        end
      when p_type == 'Opt'
        aval_opt = patt_container[p_name]
        mat = aval_opt.include?(params[p_name])
        if mat == false
          raise InvalidParam, "Unexpected option of param #{p_name} (#{params[p_name]}). Avaliable option: (#{aval_opt})."
        end
      when p_type == 'Int'
        ...
        when mode == 'BETWEEN'
          if val < min or val > max
            raise InvalidParam, "Value of #{p_name} should be between #{min} and #{max} ."
          end
        when mode == 'GE'
          if val < min
            raise InvalidParam, "Value of #{p_name} should be greater than or equal to #{min}."
          end
        when mode == 'LE'
          if val > max
            raise InvalidParam, "Value of #{p_name} should be less than or equal to #{max}."
          end
        when mode == 'EXCLUDE'
          if val >= min and val <= max
            raise InvalidParam, "Value of #{p_name} can NOT be between #{min} and #{max} .."
          end
        else
      ...
      when p_type == 'DepOpt'
        ...
        if aval_opt.nil? and pattern['Opt'][parent_param].include?(parent_val) == false
          raise  InvalidParam, "Cannot find avaliable option of [#{p_name}] by related param [#{parent_param}](Value: #{parent_val})."
...
class AioCheckOutParamVerify < PaymentVerifyBase
  ...
  def verify_aio_payment_param(params)
    if params.is_a?(Hash)
      ...
      # if param == {}
      #   raise InvalidParam, %Q{Parameter hash is empty.}
      # end
      param_diff = @aio_basic_param - params.keys
      unless param_diff == []
        raise InvalidParam, "Lack required param #{param_diff}"
      end
        ...
        if related_require_param.nil? == false and related_require_param != []
          related_require_param.each do |e|
            unless params.keys.include?(e)
              raise InvalidParam, "Lack required parameter [#{e}] when [#{pa}] is set to [#{val}] "
            end
          end
        end
      end
  ...
  def verify_aio_inv_param(params)
    if params.is_a?(Hash)
      ...
      if params.has_value?(nil)
        raise InvalidParam, %Q{Parameter value cannot be nil}
      end
      #1. 比對欄位是否缺乏
      inv_param_names = @aio_conditional_param['InvoiceMark']['Y']
      param_diff = inv_param_names - params.keys()
      unless param_diff == []
        raise InvalidParam, %Q{Lack required invoice param #{param_diff}}
      end
      ...
      if params['CarruerType'].to_s == '1'
        if params['CustomerID'].to_s.empty?
          raise InvoiceRuleViolate, "[CustomerID] can not be empty when [CarruerType] is 1."
        end
      end
      ...
      if params['Print'].to_s == '1'
        if params['CustomerName'].to_s.empty? or params['CustomerAddr'].to_s.empty?
          raise InvoiceRuleViolate, "[CustomerName] and [CustomerAddr] can not be empty when [Print] is 1."
        end
      end
      #c CustomerPhone和CustomerEmail至少一個有值
      if  params['CustomerPhone'].to_s.empty? and params['CustomerEmail'].to_s.empty?
        raise InvoiceRuleViolate, "[CustomerPhone] and [CustomerEmail] can not both be empty."
      end
      #d 別[TaxType]為 2 => ClearanceMark = 1 or 2
      if params['TaxType'].to_s == '2'
        unless ['1', '2'].include?(params['CarruerType'].to_s)
          raise InvoiceRuleViolate, "[ClearanceMark] has to be 1 or 2 when [TaxType] is 2."
        end
      end
      ...
      unless params['CustomerIdentifier'].to_s.empty?
        if ['1', '2'].include?(params['CarruerType'].to_s)
          raise InvoiceRuleViolate, "[CarruerType] Cannot be 1 or 2 when [CustomerIdentifier] is given."
        end
        unless params['Donation'].to_s == '0' and params['Print'].to_s == '1'
          raise InvoiceRuleViolate, "[Print] must be 1 and [Donation] must be 0 when [CustomerIdentifier] is given."
        end
      end
      ...
      if ['', '1'].include?(params['CarruerType'].to_s)
        unless params['CarruerNum'].to_s == ''
          raise InvoiceRuleViolate, "[CarruerNum] must be empty when [CarruerType] is empty or 1."
        end
      elsif params['CarruerType'].to_s == '2'
        if /[A-Za-z]{2}[0-9]{14}/.match(params['CarruerNum']).nil?
          raise InvoiceRuleViolate, "[CarruerNum] must be 2 alphabets and 14 numbers when [CarruerType] is 2."
        end
      elsif params['CarruerType'].to_s == '3'
        if /\/[A-Za-z0-9]/.match(params['CarruerNum']).nil?
          raise InvoiceRuleViolate, "[CarruerNum] must start with '/' followed by 7 alphabet and number characters when [CarruerType] is 3."
        end
      else
        raise InvoiceRuleViolate, "Unexpected value in [CarruerType]."
      end

      #[CarruerType]有值時，Print必須為0
      if params['CarruerType'].to_s != '' and params['Print'].to_s != '0'
      	raise InvoiceRuleViolate, "[Print] must be 0 when [CarruerType] has value."
      end

      # Donation = 1 => LoveCode不能為空, Print = 0
      if params['Donation'].to_s == '1'
        if params['LoveCode'].to_s.empty?
          raise InvoiceRuleViolate, "[LoveCode] cannot be empty when [Donation] is 1."
        end
        unless params['Print'].to_s == '0'
          raise InvoiceRuleViolate, "[Print] must be 0 when [Donation] is 1."
        end
      end
      ...
      if params['InvoiceItemName'].empty?
      	raise InvoiceRuleViolate, "[InvoiceItemName] is empty."
      else
	      if params['InvoiceItemName'].include?('|')
	        ...
	        item_params.each do |param_name|
	          ...
	          unless /(\|\||^\||\|$)/.match(params[param_name]).nil?
	            raise InvoiceRuleViolate, "[#{param_name}] contains empty value."
	          end
	          p_cnt = params[param_name].split('|').length
	          unless item_cnt == p_cnt
	            raise InvoiceRuleViolate, %Q{Count of item info [#{param_name}] (#{p_cnt}) not match item count from [InvoiceItemCount] (#{item_cnt})}
	          end
	        end
	        ...
	        unless vio_tax_t == []
	          raise InvoiceRuleViolate, "Ilegal [InvoiceItemTaxType]: #{vio_tax_t}"
	        end
	        if params['TaxType'].to_s == '9'
	          unless item_tax.include?('1')
	            raise InvoiceRuleViolate, "[InvoiceItemTaxType] must contain at lease one '1'."
	          end
	          if item_tax.include?('2') and item_tax.include?('3')
	            raise InvoiceRuleViolate, "[InvoiceItemTaxType] cannot contain 2 and 3 at the same time."
	          end
	        end
	      else
	        #沒有管線 => 逐一檢查後4項有無管線
