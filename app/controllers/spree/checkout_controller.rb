...
module Spree
  ...
  def aioall(order)
    # @order = Spree::Order.find(self.id)
order.update!(merchant_trade_no: order.created_at.strftime("%Y%m%d").to_s + order.number)
...
=end
  ...
  def notify
    ...
    if pay_params[:RtnCode] == '1'
      ...
      Spree::Payment.create(amount: @order.payment_total, order_id: @order.id, payment_method_id: 6, state: "checkout")
    end
    @order.save
    unless @order.next
      flash[:error] = @order.errors.full_messages.join("\n")
